...
def _set_description(
    meta: Dict, description: Optional[Union[Text, bool]] = None, **kwargs
) -> Dict:
    """
...
def _set_image(
    ...
    **kwargs
    ...
    if isinstance(image, Property) and image.type == PropertyType.ATTACHMENT_VALUE:
        meta.update(
            {
                MetaWidget.CUSTOM_IMAGE: "/api/v3/properties/{}/preview".format(
                    image.id
                ),
...
def _check_prefilters(
    ...
    if isinstance(prefilters, dict):
        property_models: List[Property, Text] = prefilters.get(
...
def _check_excluded_propmodels(
    ...
    if not isinstance(part_model, Part):
        raise IllegalArgumentError(
            '`part_model` must be a Part object, "{}" is not.'.format(part_model)
        )
