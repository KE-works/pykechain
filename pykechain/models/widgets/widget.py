...
class Widget(BaseInScope):
    ...
    def create(cls, json: Dict, **kwargs) -> "Widget":
        ...
        def _type_to_classname(type_widget: str):
            ...
            """
            return (
                "{}Widget".format(type_widget.title())
                if type_widget
                else WidgetTypes.UNDEFINED
            )
    ...
    def remove_associations(
        self, models: List[Union["Property", Text]], **kwargs
    ) -> None:
        """
    ...
    def _validate_excel_export_inputs(
        ...
        if file_name is None:
            ...
            if not isinstance(file_name, str):
                raise IllegalArgumentError(
                    '`file_name` must be a string, "{}" is not.'.format(file_name)
                )
        ...
        if user is not None and not isinstance(user, User):
            raise IllegalArgumentError(
                '`user` must be a Pykechain User object, "{}" is not.'.format(user)
            )
    ...
    def download_as_excel(
        ...
        if self.widget_type not in grid_widgets:
            raise IllegalArgumentError(
                "Only widgets of type {} can be exported to Excel, `{}` is not.".format(
                    grid_widgets, self.widget_type
                )
            )
        ...
        if response.status_code != requests.codes.ok:  # pragma: no cover
            raise APIError(
                "Could not export widget {}: {}".format(str(response), response.content)
            )
