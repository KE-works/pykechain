...
def check_uuid(uuid: Optional[Text], key: Optional[Text] = "pk") -> Optional[Text]:
    ...
    if uuid is not None:
        if not is_uuid(uuid):
            raise IllegalArgumentError(
                '`{}` must be a valid UUID, "{}" ({}) is not.'.format(
                    key, uuid, type(uuid)
                )
            )
    return uuid
...
def check_text(text: Optional[Text], key: Text) -> Optional[Text]:
    ...
    if text is not None:
        if not isinstance(text, str):
            raise IllegalArgumentError(
                '`{}` should be a string, "{}" ({}) is not.'.format(
                    key, text, type(text)
                )
            )
    return text
...
def check_url(url: Optional[Text], key: Text = "url") -> Optional[Text]:
    ...
    if url is not None:
        if not is_url(url):
            raise IllegalArgumentError(
                '`{}` should be a valid URL, "{}" ({}) is not.'.format(
                    key, url, type(url)
                )
            )
    return url
...
def check_list_of_text(
    list_of_text: Optional[Iterable[Text]], key: Text, unique: bool = False
) -> Optional[List[Text]]:
    ...
    if list_of_text is not None:
        if not isinstance(list_of_text, iter_types) or not all(
            isinstance(t, Text) for t in list_of_text
        ):
            raise IllegalArgumentError(
        ...
        if isinstance(dt, datetime):
            if not dt.tzinfo:
                warnings.warn(
                    ...
                    "Date will be interpreted as UTC time.".format(
            ...
            raise IllegalArgumentError(
                '`{}` should be a datetime.datetime() object, "{}" ({}) is not.'.format(
                    key, dt, type(dt)
                )
            )
    return dt
...
def check_base(
    ...
    if obj is not None:
        ...
        if isinstance(obj, cls):
            ...
            raise IllegalArgumentError(
                '`{}` must be an ID, UUID or `{}` object, "{}" ({}) is not.'.format(
                    key, cls.__name__, obj, type(obj)
                )
            )
    return obj
    ...
    if obj is not None:
        ...
        if isinstance(obj, cls):
            ...
            try:
                ...
                raise IllegalArgumentError(
                    '`{}` must be an ID or `{}` object, "{}" ({}) is not.'.format(
                        key, cls.__name__, obj, type(obj)
                    )
                )
        elif method:
