...
from pykechain.models.input_checks import (
    check_list_of_base, check_list_of_dicts, check_text, check_type
)
...
class Part(TreeObject):
    ...
    def property(self, name: str = None) -> "AnyProperty":
        ...
        if not matches:
            raise NotFoundError(
                f"Could not find a property with name, id or ref: {name}"
            )
        elif len(matches) > 1:
            raise MultipleFoundError(
                f"Found multiple properties with name, id or ref: {name}"
            )
        else:
            return matches[0]
