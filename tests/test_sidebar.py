...
class TestSideBar(TestBetamax):
    def setUp(self):
        ...
        self.scope_activity_uri = "{}/{}".format(
            self.scope_uri, SubprocessDisplayMode.ACTIVITIES
        )
    ...
    def test_add_buttons(self):
        buttons = [
            {
                ...
                "uri": "{}/{}".format(
                    self.scope_activity_uri, self.scope.activities()[0].id
                ),
            },
            {
        ...
        new_buttons = self.manager.add_buttons(buttons=buttons, override_sidebar=True,)
    ...
    def test_bulk_creation(self):
        ...
        bulk_creation_manager.add_task_button(
            activity=self.scope.activities()[0], icon="bookmark",
        )
        bulk_creation_manager.add_external_button(
            url="https://www.google.com", icon="google", title="Google",
        )
        bulk_creation_manager.add_ke_chain_page(
            page_name=KEChainPages.WORK_BREAKDOWN, icon="site-map"
        )
