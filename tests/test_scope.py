...
class TestScopeMembers(TestBetamax):
    ...
    DEFAULTS = {
        ScopeRoles.MANAGER: ["superuser", "testmanager",],
        ScopeRoles.LEADMEMBER: ["superuser", "testlead",],
        ScopeRoles.MEMBER: ["testuser",],
    }
    ...
    def _reset_members(self):
        ...
        for role in [ScopeRoles.MANAGER, ScopeRoles.LEADMEMBER, ScopeRoles.MEMBER]:
            for member in self.DEFAULTS[role]:
                self.project._update_scope_project_team(
                    ScopeMemberActions.ADD, role, member
                )
    ...
    def test_members(self):
        ...
        self.assertTrue(
            basic_members and leads and managers and lead_and_manager,
    ...
    def test_add_member(self):
        ...
        # testing
        self.assertTrue(
            member_to_be_added in [member["username"] for member in project_members]
        )
    ...
    def test_remove_member(self):
        ...
        self.assertTrue(
            ...
            not in [member["username"] for member in project_members]
        )
    ...
    def test_remove_manager(self):
        ...
        project_managers = self.project.members(is_manager=True)
        self.assertTrue(
            manager_to_be_removed
            not in [manager["username"] for manager in project_managers]
        )
        project_managers = self.project.members(is_manager=False)
        self.assertTrue(
            manager_to_be_removed
            in [manager["username"] for manager in project_managers]
        )
...
class TestScopeEdit(TestBetamax):
    ...
    def test_edit_scope(self):
        ...
        retrieved_project = self.client.scope(
            id=self.scope.id, status=ScopeStatus.CLOSED
        )
