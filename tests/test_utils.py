...
class TestIsURL(SixTestCase):
    def test_is_url_returns_true_on_valid_url(self):
        ...
        for address in addresses:
            with self.subTest(address):
                self.assertTrue(
                    is_url(address), "should be a valid address: '{}'".format(address)
                )
    ...
    def test_is_url_returns_False_on_failed_url(self):
        ...
        for address in failed_addresses:
            with self.subTest(address):
                self.assertFalse(
                    is_url(address), "should be a invalid address: '{}'".format(address)
                )
        ...
        for email in valid_addresses:
            with self.subTest(email):
                self.assertTrue(
                    ...
                    "should be a valid address: '{}'".format(email),
                )
    ...
    def test_is_email_returns_false_on_invalid_url(self):
        ...
        for email in invalid_addresses:
            with self.subTest(email):
                self.assertFalse(
                    is_valid_email(email),
                    "should be an invalid address: '{}'".format(email),
                )
