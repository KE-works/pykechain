...
class TestClient(TestCase):
    ...
    def test_init_token(self):
        ...
        self.assertTrue(
            client.headers["Authorization"], "Token {}".format(PSEUDO_TOKEN)
        )
        self.assertIsNone(client.auth)
...
class TestCloneScopeAsync(TestBetamax):
    def setUp(self):
        super().setUp()
        self.temp_scope = None
        self.source = self.client.create_scope(name="_Async to be cloned scope SOURCE",)
        time.sleep(3)
    ...
    def test_clone_asynchronous(self):
        ...
        self.client.clone_scope(
            name=clone_name, source_scope=self.source, asynchronous=True,
        )
