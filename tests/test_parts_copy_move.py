...
class TestPartsCopyMove(TestBetamax):
    ...
    def setUp(self):
        ...
        self.front_wheel, self.rear_wheel = (
            self.project.part(name="Front Wheel"),
            self.project.part(name="Rear Wheel"),
        )
        self.specify_wheel_diameter = self.project.activity(
            name="Specify wheel diameter"
        )
            ...
            "features": [
                {
                    ...
                    "geometry": {
                        "type": "Point",
                        "coordinates": [4.390912, 51.9929856],
                    },
                    "properties": {
                        "zoom": 16,
        ...
        self.p_geojson_value_options = {
            "representations": [
                {
                    "rtype": "geoCoordinate",
                    "config": {"geoCoordinate": "approx_address"},
                }
            ]
        }
        ...
        self.model_to_be_copied.property(
            self.p_part_reference_name
    ...
    def test_copy_part_model_given_name_include_children(self):
        ...
        self.assertEqual(
            len(copied_model.properties), len(self.model_to_be_copied.properties)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            copied_model.property(name=self.p_char_name)._json_data["unit"], "mm"
        )
        self.assertEqual(
            copied_model.property(name=self.p_int_name).value, self.p_int_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_char_name).value, self.p_char_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_multi_name).value, self.p_multi_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_int_name).value, self.p_int_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_float_name).value, self.p_float_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_bool_name).value, self.p_bool_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_date_name).value, self.p_date_value
        )
        self.assertIn(
            self.p_datetime_value,
            copied_model.property(name=self.p_datetime_name).value,
        )
        self.assertIn(
            copied_model.property(name=self.p_attach_name).filename, self.p_attach_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_link_name).value, self.p_link_value
        )
        self.assertEqual(
            copied_model.property(name=self.p_single_select_name).value,
            self.p_single_select_value,
        )
        self.assertEqual(
            copied_model.property(name=self.p_multi_select_name).value,
            self.p_multi_select_value,
        )
        list_of_user_ids = [
            user.id
            for user in copied_model.property(name=self.p_user_reference_name).value
        ]
        self.assertIn(
            self.sub_part2.name,
            [
                part.name
                for part in copied_model.property(name=self.p_part_reference_name).value
            ],
        )
        self.assertNotEqual(
            self.sub_part2.id,
            copied_model.property(name=self.p_part_reference_name).value[0].id,
        )
        self.assertIn(
            self.wheel.id,
            [
        ...
        self.assertIn(
            ...
            [
                act.id
                for act in copied_model.property(
                    name=self.p_activity_reference_name
                ).value
            ],
        )
        self.assertIn(
            ...
            [
                scope.id
                for scope in copied_model.property(
                    name=self.p_project_reference_name
                ).value
            ],
        )
    ...
    def test_copy_part_model_include_instances(self):
        ...
        copied_instance = [
            child for child in bike_part.children() if "Copied" in child.name
        ][0]
        ...
        self.assertEqual(
            copied_instance.property(name=self.p_char_name).value, self.p_char_value
        )
        self.assertEqual(
            copied_instance.property(name=self.p_multi_name).value, self.p_multi_value
        )
        self.assertEqual(
            copied_instance.property(name=self.p_int_name).value, self.p_int_value
        )
        self.assertEqual(
            copied_instance.property(name=self.p_float_name).value, self.p_float_value
        )
        self.assertEqual(
            copied_instance.property(name=self.p_bool_name).value, self.p_bool_value
        )
        self.assertEqual(
            copied_instance.property(name=self.p_date_name).value, self.p_date_value
        )
        self.assertIn(
            self.p_datetime_value,
            copied_instance.property(name=self.p_datetime_name).value,
        )
        self.assertEqual(
            copied_instance.property(name=self.p_link_name).value, self.p_link_value
        )
        self.assertEqual(
        ...
        list_of_user_ids = [
            user.id
            for user in copied_instance.property(name=self.p_user_reference_name).value
        ]
        self.assertIn(
            self.sub_part2.name,
            [
        ...
        self.assertDictEqual(
            self.p_geojson_value,
            copied_instance.property(name=self.p_geo_json_name).value,
        )
        self.assertDictEqual(
            self.p_weather_value,
            copied_instance.property(name=self.p_weather_name).value,
        )
        self.assertIn(
            self.specify_wheel_diameter.id,
            [
                act.id
                for act in copied_instance.property(
                    name=self.p_activity_reference_name
                ).value
            ],
        )
        self.assertIn(
            ...
            [
                scope.id
                for scope in copied_instance.property(
                    name=self.p_project_reference_name
                ).value
            ],
        )
    ...
    def test_copy_internal_references_on_model(self):
        ...
        reference_property = self.dump_part.property(
            name="__Property internal reference"
        )
    ...
    def test_copy_part_instance(self):
        ...
        instance_to_be_copied.property(
            name=self.p_part_reference_name_cross_scope
        ).value = [self.front_wheel, self.rear_wheel]

        instance_target_parent = self.project.part("Bike")
        copied_instance = instance_to_be_copied.copy(
            target_parent=instance_target_parent,
            name="__Copied instance",
            include_children=True,
        )
        copied_instance.populate_descendants()
        self.dump_part = copied_instance.model()
    ...
    def test_copy_different_categories(self):
        ...
        with self.assertRaisesRegex(
            IllegalArgumentError, "must have the same category"
        ):
    ...
    def test_cross_scope_copy(self):
        ...
        self.dump_part.refresh()
        copied_options = self.dump_part.property(
            self.p_part_reference_name_cross_scope
        )._options
        self.assertTrue(
            self.model_to_be_copied.scope_id == copied_options.get("scope_id")
        )

        copied_sub_part2_int_prop = self.dump_part(self.sub_part2.name).property(
            self.p_int_name
        )
        copied_sub_part2_bool_prop = self.dump_part(self.sub_part2.name).property(
            self.p_bool_name
        )
        ...
        self.assertTrue(
            copied_sub_part2_bool_prop.id in copied_options.get("propmodels_excl")
        )
        self.assertTrue(
            copied_sub_part2_int_prop.id
            in copied_options["prefilters"]["property_value"]
        )
