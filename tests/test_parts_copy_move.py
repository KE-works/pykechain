...
class TestPartsCopyMove(TestBetamax):
    def setUp(self):
        ...
        self.model_to_be_copied = self.project.create_model(
            parent=self.base,
            name="__Model to be Copied @ {} [TEST]".format(str(datetime.now())),
        ...
        sub_part2 = self.model_to_be_copied.add_model(
            name="__subpart 2", multiplicity=Multiplicity.ONE
        )
    ...
    def test_copy_part_model_given_name_include_children(self):
        ...
        self.assertEqual(
            copied_model.property("__Property single text")._json_data["description"],
            "Description of Property single text",
        )
        self.assertEqual(
            copied_model.property("__Property single text")._json_data["unit"], "mm"
        )
        self.assertEqual(copied_model.property("__Property decimal number").value, 33)
        self.assertEqual(
            copied_model.property("__Property single select list").options,
            ["a", "b", "c"],
        )
    ...
    def test_copy_part_model_include_instances(self):
        ...
        copied_instance = [
            child for child in bike_part.children() if "to be Copied" in child.name
        ][0]
        ...
        self.assertEqual(
            copied_instance.property("__property-decimal-number").value, 33
        )
        self.assertEqual(
            copied_instance.property("__property-single-select-list").value, None
        )
    ...
    def test_move_part_model(self):
        ...
        self.assertEqual(
            clone_of_original_model.name,
            "CLONE - {}".format(self.model_to_be_copied.name),
        )

        clone_of_original_model.move(
            target_parent=model_target_parent,
    ...
    def test_move_part_instance(self):
        ...
        multiplicity_part1 = self.instance_to_be_copied.add(
            model=multiplicity_model, name="__multiplicity part instance 1",
        )
        multiplicity_part2 = self.instance_to_be_copied.add(
            model=multiplicity_model, name="__multiplicity part instance 2",
        )
