...
class TestParts(TestBetamax):
    ...
    def test_base_comparison(self):
        ...
        self.assertFalse(
            all_parts[0] == all_parts[1],
            msg="2 different parts must not be equal." + tip,
        )
        self.assertTrue(
            all_parts[0] == all_parts[0], msg="The same part must be equal." + tip
        )
        self.assertFalse(
            all_parts[0] == 5,
    ...
    def test_base_hash(self):
        ...
        try:
            ...
            self.assertTrue(
                False, msg="Parts must be hashable See Base.__hash__() method."
            )
    ...
    def test_wrongly_create_model(self):
        ...
        with self.assertRaises(IllegalArgumentError):
            self.client.create_model(
                name="Multiplicity does not exist",
                parent=bike_model,
                multiplicity="TEN",
            )
    ...
    def test_create_part_where_model_is_instance(self):
        ...
        with self.assertRaises(IllegalArgumentError):
            self._part = self.client.create_part(
                name="Model should not be instance",
    ...
    def test_create_proxy_model_where_parent_is_instance(self):
        ...
        with self.assertRaises(IllegalArgumentError):
            self._part = self.client.create_proxy_model(
                name="Parent should not be instance",
                model=bike_model,
                parent=bike_instance,
            )
    ...
    def test_add_proxy_to(self):
        ...
        all_bearing_catalog_models = self.project.parts(
            name="Bearing", category=Category.MODEL
        )
    ...
    def test_retrieve_non_existent_proxies_of_a_catalog_model_raises_error(self):
        ...
        catalog_model = self.project.model(
            "Model", classification=Classification.CATALOG
        )
        with self.assertRaises(NotFoundError):
    ...
    def test_retrieve_part_multiplicity(self):
        bike_model = self.project.model("Bike")
        self.assertEqual(
            bike_model.multiplicity, Multiplicity.ONE_MANY, bike_model.multiplicity
        )
...
class TestBulkPartsCreation(TestBetamax):
    def setUp(self):
        ...
        self.part_model = self.project.create_model(
            name="Part", parent=self.product_model
        )
    ...
    def test_bulk_create_parts(self):
        ...
        for part_created in parts_created:
            ...
            self.assertEqual(
                part_created.property(name=self.text_prop.name).value, str(idx)
            )
            self.assertEqual(
                part_created.property(name=self.char_prop.name).value, str(idx)
            )
            self.assertEqual(part_created.property(name=self.int_prop.name).value, idx)
            self.assertEqual(
                part_created.property(name=self.float_prop.name).value,
                float(f"{idx}.{idx}"),
            )
            self.assertEqual(
            ...
            self.assertEqual(
                part_created.property(name=self.link_prop.name).value,
                f"http://{idx}.com",
            )
            self.assertEqual(
                part_created.property(name=self.ss_prop.name).value, f"{idx}"
            )
            self.assertEqual(
                part_created.property(name=self.ms_prop.name).value, [f"{idx}"]
            )
            self.assertEqual(
                part_created.property(name=self.part_ref_prop.name).value[0].name,
                "Front Wheel",
            )
            self.assertEqual(
            ...
            self.assertEqual(
                part_created.property(name=self.weather_prop.name).value, dict()
            )
            self.assertEqual(
                part_created.property(name=self.geo_info_prop.name).value, dict()
            )
